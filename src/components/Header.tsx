...
// Helper to get logo and name from settings
function useSiteLogoAndName() {
  const { settings } = useSiteSettings();
  return {
    logo: settings.site_logo || "/lovable-uploads/a5f88509-5d42-4d11-8b7c-6abe9e64cfd0.png",
...
const Header = () => {
  ...
  const { logo, name } = useSiteLogoAndName();
        ...
        <div className="flex justify-between items-center">
          ...
          >
            <img
              ...
              loading="lazy"
            />
            <span className="text-portfolio-purple">{name}</span>
          </a>
          ...
          >
            <div className="flex justify-between items-center mb-12">
              <a href="#" className="flex items-center gap-2 text-2xl font-bold font-orbitron">
                <img src={logo} alt="Site Logo" className="h-10 w-10 object-contain rounded-full border-2 border-purple-500 bg-white shadow" />
                <span className="text-portfolio-purple">{name}</span>
              </a>
              <button
            ...
            <nav className="flex flex-col space-y-8">
              {navLinks.map((link, index) => (
                <a
                  key={index}
                  href={link.href}
                  className="text-xl text-gray-300 hover:text-purple-400 transition-all duration-300 font-medium py-3 border-b border-purple-500/20 hover:border-purple-400/50"
                  onClick={() => setIsMenuOpen(false)}
